...
if [ -z "$USERNAME" ]
then
    echo -e "User name not set please provide one, Not recommended usung just th root account"
fi
### user password check
if [ -z "$USERPASS" ]
then
    echo -e "User password not set Recommend setting a password for security"
fi
### root same as user password scheck
if [ -z "RSAUPASS" ]
then
    echo -e ""
else
    echo -e "Not reccomended using User password as Root password"
fi
### Root password check
